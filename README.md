...
- тарифы: 7 / 30 / 90 дней;
- статус подписки с понятным next step (оплатить / открыть подключение / проверить активацию);
- получение `vpn://` и `.conf`;
- self-service перевыпуск текущего устройства через `/resetdevice` (с подтверждением);
- инструкция и контакт поддержки;
- короткие команды `/support`, `/paysupport`, `/terms`;
- простой referral entry через deep-link и короткое правило бонуса после первой оплаты друга.
...
- поиск пользователя: `/finduser QUERY` (id / username);
- краткая платёжная сводка для оператора: `/payinfo USER_ID` + refund-prep (manual only).
...
## Single-server scope (важно)

Эта ветка (`selfhost`) намеренно остаётся в модели **один бот → один AWG target/server**.

- multi-node и multi-country маршрутизация **не входят** в текущую задачу;
- логика второго сервера осознанно отложена до роста нагрузки.

### TODO (отложено)
- добавить второй сервер / вторую страну после фактического роста (ориентир: ~50+ активных пользователей);
- выполнить это отдельной задачей, без усложнения текущего personal MVP.
