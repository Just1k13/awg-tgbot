...
Если найдены остаточные файлы старой установки без systemd-сервиса:
...
1. `Удалить всё` — удаляет код, базу, `.env`, сервис и логи.
2. `Удалить всё, кроме БД и .env` — удаляет код, сервис, venv и логи, но сохраняет базу и `.env`, чтобы не потерять `ENCRYPTION_SECRET` и возможность расшифровать данные в БД.
...
- `config.py` не делает интерактивный ввод при импорте.
- Установщик ждёт освобождения `apt/dpkg lock`, если пакетный менеджер занят `unattended-upgrades`.
